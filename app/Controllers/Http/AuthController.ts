...
export default class AuthController {
    ...
    public async validateToken({ auth, response }: HttpContextContract) {
        try {
            ...
            if (user) {
                return response.json({ isValid: true, profile_id: user.profile_id });
            } else {
    ...
    public async resetPassword({ request, response }: HttpContextContract) {
        try {
            let { token, password } = request.only(['token', 'password']);
            ...
            // Primeiro, tente encontrar o registro de reset que ainda não expirou
            const passwordReset = await PasswordReset.query()
                .where('expires_at', '>', DateTime.now().toSQL())
                .first();

            // Se não encontrar o token (ou se estiver expirado), retorne um erro específico
            if (!passwordReset || !(await Hash.verify(passwordReset.token, token))) {
